...
function generate_new_transaction_number() {
	...
	//create a new order
	$sql = "INSERT INTO orders(user_id, transcation_code, purchase_date, status_id, payment_mode_id) VALUES ('$user_id', '$transaction_number', '$purchase_date', '$status_id', '$payment_mode_id'); ";
...
$staff_email = 'weego.onlinestore@gmail.com';
$customer_email = $_SESSION['user']['email'];          //
$subject = 'WeeGO - Order Confirmation';
$body = '<div style="text-transform:uppercase;"><h3>Reference No.: '.$transaction_number.'</h3></div>'."<div>Ship to $address</div>". "<div></div>";
try {
    //Server settings
    ...
    $mail->Password = 'sunnyjhunibabao';                     // SMTP password
    $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
    $mail->Port = 587;                                    // TCP port to connect to

    //Recipients
    $mail->setFrom($staff_email, 'WeeGo');
